; By KevinM. SJC, added goal tape check at $1493

; If set to an address, the item box/item will only be drawn when you an item if this address holds a non-zero value.
...
; Keep this at 0 to draw it in every level when you get item.
!freeram = $0E0A

; 0 = Whenever it's enabled, only draw the box if there's an item.
; 1 = Whenever it's enabled, draw the box even if there's not an item
!always_draw = 0
...
main:
    ...
    ; Don't draw if goal tape. SJC addition
    lda $1493 : bne .return

;if !freeram, removed condition, SJC
