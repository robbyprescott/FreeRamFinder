...
;;
;;Comes with On/Off and mini versions (by yoshicookiezeus)
;;
...
;;Extra Bit - yes
;;
;;If set, it's movement will depend on On/Off state.
;;
...
!OnOffState = 0			;0 - it'll only move when On/Off switch is ON. anything else is when it's OFF.
...
AND #$04			;
BEQ .Continue			;if not set, no On/Off dependance

LDA $14AF|!Base2		;check on/off

If !OnOffState = 0		;
  BEQ .Continue			;move when on
else				;
  BNE .Continue			;move when off
endif				;
RTS
