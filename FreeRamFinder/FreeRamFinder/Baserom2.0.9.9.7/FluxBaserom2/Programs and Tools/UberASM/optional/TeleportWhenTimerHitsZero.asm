...
; This code's actual custom countdown function is only used if you set !KillVanillaFunction = 1, 
; since otherwise the vanilla counter is actually still enabled (and it'd count down twice otherwise).
; It'll still work fine if you don't do this; but when the timer transitions 
; from 20 to 19 or 10 to 9, it's slightly graphically awkward.

; Again, though, this can be fixed if !KillVanillaFunction = 1. 
; Note, however, that if you do this, it'll disable other counting functions in your level, like coin count. 

; Also, note that by default, (sub)levels accessed through the level hub 
; will still use the timer from the parent/main level

!KillVanillaFunction = 1
...
tiles:
    db $29,$30,$31,$DD   ; 29 is 2-up. 30 and 31 - cluster leaf and fish. DD = item reserve box
