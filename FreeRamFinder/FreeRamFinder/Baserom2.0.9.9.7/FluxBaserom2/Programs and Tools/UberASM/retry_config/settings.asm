...
; Only played if not playing the death song (for example, it's not played if the level uses vanilla death).
    !death_sfx = $2D ; $20 is Yoshi spit sound
...
; By default in the Flux baserom, the game is (silently) saved any time you're on the OW. 																						  
...
; Note: you can also change these on the fly (see "docs/ram_map.txt").
    !text_x_pos = $B8  ; middle: $58. Bottom left: $08
    !text_y_pos = $D0  ; middle: $6F. Bottom left: $D0
...
; Palette row used by the letters and cursor (note: they use sprite palettes).
    !letter_palette = $09 ; default $08
    !cursor_palette = $0A ; default $08
...
; Also, obviously these aren't used if you don't use the Retry prompt.
    !tile_curs = $20 ; + 21 
    !tile_blk  = $30 ; black, but ONLY used if !no_prompt_box = 0. (30/31 are cluster sprites)
    !tile_r    = $68 ; next three, overwrites sleepy ZZZs sprite if 4E, 4F, 5E, 5F...
    !tile_e    = $69 ; overwrites rain cluster
    !tile_t    = $78 
    !tile_y    = $79 ; 
    !tile_x    = $29 ; overwrites 2-up if gotten while die. You only need this and the next one if you change to include EXIT 
    !tile_i    = $5F ; rip van?
